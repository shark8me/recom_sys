...
public class TFIDFModelBuilder implements Provider<TFIDFModel> {
    ...
    public TFIDFModel get() {
        ...
        //System.out.println("has 884? "+tagIds.values().contains(884L)+" has key "+tagIds.containsKey("3")+" "+tagIds.get("3"));
        for (long item: items) {
           ...
           //System.out.println(" item is "+item);
            ...
            for(String s:tags){
            	...
            	//System.out.println(" tag is "+s+" key is "+key);
            }
            for(Long s:counts.keySet()){
            	work.set(s,counts.get(s));
            }
        ...
        for (VectorEntry e: docFreq.fast()) {
        	...
        	double d=Math.log(100/e.getValue());
        	docFreq.set(e, d);
        }
...
//            	//System.out.println("key "+ve.getKey()+" value "+ve.getValue()+" from df "+docFreq.get(ve.getKey()));
//            	tv.set(ve.getKey(), ve.getValue()*docFreq.get(ve.getKey()));
//            }
            //System.out.println(" key "+entry.getKey()+" spv "+tv);
        ...
        //System.out.println(" key "+itemVectors.get(11L));
