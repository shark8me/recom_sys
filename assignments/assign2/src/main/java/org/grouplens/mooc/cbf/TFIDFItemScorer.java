...
public class TFIDFItemScorer extends AbstractItemScorer {
    ...
    public void score(long user, @Nonnull MutableSparseVector output) {
        ...
        for (VectorEntry e: output.fast(VectorEntry.State.EITHER)) {
            ...
            double res=cos.similarity(userVector, iv);
            output.set(e, res);
        }
    }

    //my arguments
    //1275 2119 159 351 1632
    private SparseVector makeUserVector(long user) {
        ...
        int numRatings=userRatings.size();
        double sumRatings=0;
        for (Rating r: userRatings) {
        	sumRatings+=r.getPreference().getValue();
        }
        double userAvg=sumRatings/numRatings;
...
//            }
            ...
            if (p != null) {
                // The user likes this item!
                // TODO Get the item's vector and add it to the user's profile
            	SparseVector sv=model.getItemVector(p.getItemId());
            	double wt=p.getValue()-userAvg;
            	MutableSparseVector msv=sv.mutableCopy();
            	msv.multiply(wt);
            	profile.add(msv);
            }
        }
